...
var SUPP_SCHEDULE = [
  { time: "MORNING (with food)", items: [
    { id: "multi", name: "Multivitamin", dose: "1 tablet" },
    { id: "d3", name: "Vitamin D3", dose: "4000IU" },
    { id: "k2", name: "K2 MK-7", dose: "100-200mcg" },
    { id: "omega3", name: "Omega-3", dose: "2-3 softgels" },
    { id: "selenium", name: "Selenium", dose: "200mcg" },
    { id: "lions", name: "Lions Mane", dose: "1 serving", note: "cycle: 4-6 weeks on, 2-4 off" },
    { id: "spiru", name: "Spirulina", dose: "3-5g" },
    { id: "ashwa", name: "Ashwagandha", dose: "1 serving", note: "cycle: 4-6 weeks on, 2-4 off" },
  ]},
  { time: "TRAINING / MORNING WATER", items: [
    { id: "creatine", name: "Creatine", dose: "6g" },
    { id: "citrul", name: "L-Citrulline", dose: "6g" },
  ]},
  { time: "ANY TIME (with water)", items: [
    { id: "psyll", name: "Psyllium Husk", dose: "5-10g" },
  ]},
  { time: "EVENING (with food)", items: [
    { id: "mag", name: "Magnesium Bisglycinate", dose: "113mg" },
    { id: "zinc", name: "Zinc Citrate", dose: "15mg" },
    { id: "glycine", name: "Glycine", dose: "3-5g" },
  ]},
  { time: "AS NEEDED ONLY", items: [
    { id: "charcoal", name: "Activated Charcoal", dose: "occasional", note: "bloating only, never within 2hrs of other supps" },
  ]},
];
...
function getISOWeekKey(dateStr) {
  ...
  var mon = new Date(d); mon.setDate(diff); return mon.toISOString().split("T")[0];
}
function computeAllWeekKeys(workouts) {
  var keys = new Set(); Object.keys(workouts).forEach(function(dk) { keys.add(getISOWeekKey(dk)); }); return Array.from(keys).sort();
}
function computePerfectWeeks(workouts) {
  var weekKeys = computeAllWeekKeys(workouts); var results = {};
...
function computeHistoricalTotals(workouts) {
  ...
  Object.values(workouts).forEach(function(da) { da.forEach(function(w) { if (totals[w.type]) { totals[w.type].sessions++; if (w.duration) totals[w.type].minutes += w.duration; } }); }); return totals;
}
...
function getWeightComparison(kg) {
  var items = [
    { name: "house cat", kg: 4.5 },{ name: "golden retriever", kg: 32 },{ name: "adult human", kg: 80 },{ name: "gorilla", kg: 160 },{ name: "grand piano", kg: 480 },{ name: "horse", kg: 500 },{ name: "white rhino", kg: 2300 },{ name: "elephant", kg: 6000 },{ name: "London bus", kg: 12500 },{ name: "T-Rex", kg: 8000 },{ name: "cement truck", kg: 33000 },{ name: "M1 Abrams tank", kg: 60000 },{ name: "Boeing 737", kg: 41000 },{ name: "blue whale", kg: 140000 },{ name: "Space Shuttle", kg: 78000 },{ name: "Statue of Liberty", kg: 204000 },{ name: "Eiffel Tower", kg: 7300000 },{ name: "cruise ship", kg: 100000000 },
  ];
  var valid = items.filter(function(it) { return kg / it.kg >= 0.5; });
  var seed = Math.floor(Date.now() / 86400000); var pick1 = valid[seed % valid.length]; var pick2 = valid[(seed + 3) % valid.length];
  ...
  if (!pick1) return ""; if (pick1 === pick2 || !pick2) return "= " + fmt(pick1); return "= " + fmt(pick1) + "  /  " + fmt(pick2);
}
function getMonthDays(year, month) {
  var first = new Date(year, month, 1); return { startDay: first.getDay() === 0 ? 6 : first.getDay() - 1, daysInMonth: new Date(year, month + 1, 0).getDate() };
}
...
function RestTimer(props) {
  var seconds = props.seconds; var onDone = props.onDone;
  ...
  useEffect(function() {
    if (running && remaining > 0) {
      intervalRef.current = setInterval(function() {
        setRemaining(function(prev) { if (prev <= 1) { clearInterval(intervalRef.current); setRunning(false); setFinished(true); try { navigator.vibrate && navigator.vibrate([200, 100, 200]); } catch (e) {} return 0; } return prev - 1; });
      }, 1000);
    }
  ...
  return (
    <div style={st.timerWrap}>
      <div style={st.timerBarBg}><div style={Object.assign({}, st.timerBarFill, { width: pct + "%", background: finished ? "#9FCB7B" : "#7FA36B" })} /></div>
      <div style={st.timerRow}>
        <span style={Object.assign({}, st.timerText, { color: finished ? "#9FCB7B" : "#D7E0D2" })}>{finished ? "REST DONE" : fmtTimer(remaining) + " / " + fmtTimer(seconds)}</span>
        {!running && !finished && <button style={st.timerStartBtn} onClick={function() { setRunning(true); }}>START</button>}
        {running && <button style={st.timerSkipBtn} onClick={function() { clearInterval(intervalRef.current); setRunning(false); setFinished(true); }}>SKIP</button>}
        {finished && <button style={st.timerSkipBtn} onClick={onDone}>OK</button>}
      </div>
    </div>
  );
}
...
export default function WorkoutTracker() {
  ...
  var _sc = useState({}), suppChecks = _sc[0], setSuppChecks = _sc[1];

  useEffect(function() {
    try {
      var w = localStorage.getItem("operator-workouts"); if (w) setWorkouts(JSON.parse(w));
      var bw = localStorage.getItem("operator-bodyweight"); if (bw) setBodyweightData(JSON.parse(bw));
      var lw = localStorage.getItem("operator-last-weights"); if (lw) setLastWeights(JSON.parse(lw));
      var sc = localStorage.getItem("operator-supps-" + getTodayKey()); if (sc) setSuppChecks(JSON.parse(sc));
    } catch (e) {}
    setLoading(false);
  }, []);

  var saveWorkouts = useCallback(function(data) { setWorkouts(data); try { localStorage.setItem("operator-workouts", JSON.stringify(data)); } catch (e) {} }, []);
  var saveBodyweight = useCallback(function(data) { setBodyweightData(data); try { localStorage.setItem("operator-bodyweight", JSON.stringify(data)); } catch (e) {} }, []);
  var saveLastWeights = useCallback(function(data) { setLastWeights(data); try { localStorage.setItem("operator-last-weights", JSON.stringify(data)); } catch (e) {} }, []);
  var saveSuppChecks = useCallback(function(data) { setSuppChecks(data); try { localStorage.setItem("operator-supps-" + getTodayKey(), JSON.stringify(data)); } catch (e) {} }, []);

  var toggleSupp = function(id) { var updated = Object.assign({}, suppChecks); updated[id] = !updated[id]; saveSuppChecks(updated); };

  var addBodyweight = function() {
    var val = parseFloat(bwInput); if (isNaN(val) || val <= 0) return;
    saveBodyweight([].concat(bodyweightData.filter(function(e) { return e.date !== bwDateInput; }), [{ date: bwDateInput, weight: val }]).sort(function(a, b) { return a.date.localeCompare(b.date); }));
    setBwInput("");
  };
  ...
  var handleSetDone = function(exIdx, si) {
    ...
    if (!wasDone) {
      ...
      if (w && String(w).trim() !== "" && ex.name) { var upd = Object.assign({}, lastWeights); upd[ex.name] = String(w).trim(); saveLastWeights(upd); }
  ...
  var suppTotal = SUPP_SCHEDULE.reduce(function(s, g) { return s + g.items.length; }, 0);
  var suppDone = SUPP_SCHEDULE.reduce(function(s, g) { return s + g.items.filter(function(it) { return suppChecks[it.id]; }).length; }, 0);
  ...
  if (activeWorkout) {
    ...
    var blocks = []; var currentBlock = null;
    activeWorkout.exercises.forEach(function(ex, i) {
      var bi = ex.blockIndex !== undefined ? ex.blockIndex : i;
      if (!currentBlock || currentBlock.blockIndex !== bi) { currentBlock = { blockIndex: bi, label: ex.block || "EXERCISE", rest: ex.restSeconds || 0, exercises: [] }; blocks.push(currentBlock); }
    ...
    return (
      <div style={st.shell}>
        ...
        <div style={Object.assign({}, st.detailBanner, { borderLeftColor: type ? type.color : "#2E3A31" })}>
          <div style={st.detailTop}><span style={{ color: type ? type.color : "#999", fontSize: 14, fontWeight: 700, marginRight: 6 }}>{type ? type.icon : ""}</span><span style={st.detailLabel}>{activeWorkout.label}</span></div>
          <div style={st.detailDate}>{DAYS[getDayIndex(activeWorkout.date)]} -- {activeWorkout.date}</div>
        </div>
        <div style={st.metaRow}>
          <div style={st.metaBlock}><div style={st.metaTag}>DURATION (MIN){isEngine ? " *" : ""}</div>
            <input type="number" style={Object.assign({}, st.metaInput, { borderColor: isEngine ? "#6B9A8A" : "#2E3A31" })} placeholder="min" value={activeWorkout.duration || ""} onChange={function(e) { updateWorkout(activeWorkout.date, activeWorkout.id, { duration: parseInt(e.target.value) || null }); }} />
            {isEngine && <div style={{ fontSize: 8, color: "#6B9A8A", marginTop: 2, letterSpacing: 1 }}>COUNTS TO 90MIN TARGET</div>}
          </div>
          <div style={Object.assign({}, st.metaBlock, { flex: 1 })}><div style={st.metaTag}>NOTES</div>
            <input type="text" style={Object.assign({}, st.metaInput, { width: "100%" })} placeholder="Session notes..." value={activeWorkout.notes} onChange={function(e) { updateWorkout(activeWorkout.date, activeWorkout.id, { notes: e.target.value }); }} />
          </div>
        </div>
        <div style={st.exList}>
          {blocks.map(function(blk, bi) { return (
            <div key={bi} style={st.blockCard}>
              <div style={st.blockHeader}><span style={st.blockLabel}>{blk.label}</span><span style={st.blockRest}>rest: {blk.rest >= 60 ? Math.round(blk.rest / 60) + "m" + (blk.rest % 60 > 0 ? blk.rest % 60 + "s" : "") : blk.rest + "s"}</span></div>
              {blk.exercises.map(function(ex, ei) { return (
                <div key={ex.id}>
                  {ei > 0 && <div style={st.supersetDivider}><span style={st.supersetTag}>SUPERSET</span></div>}
                  <div style={st.exInner}>
                    <div style={st.exHead}><input type="text" style={st.exName} value={ex.name} placeholder="Exercise name" onChange={function(e) { updateExercise(ex._idx, { name: e.target.value }); }} /><button style={st.exDel} onClick={function() { deleteExercise(ex._idx); }}>x</button></div>
                    <div style={st.setLabels}><span style={Object.assign({}, st.setLbl, { width: 28 })}>#</span><span style={st.setLbl}>REPS</span><span style={st.setLbl}>KG</span><span style={Object.assign({}, st.setLbl, { width: 52 })}></span></div>
                    {ex.sets.map(function(set, si) { return (
                      <div key={si} style={Object.assign({}, st.setRow, { background: set.done ? "rgba(127,163,107,0.06)" : "transparent" })}>
                        ...
                        <input type="text" style={st.setIn} value={set.reps} placeholder="--" onChange={function(e) { updateSet(ex._idx, si, { reps: e.target.value }); }} />
                        <input type="text" style={st.setIn} value={set.weight} placeholder="--" onChange={function(e) { updateSet(ex._idx, si, { weight: e.target.value }); }} />
                        <div style={{ display: "flex", gap: 3 }}>
                          <button style={Object.assign({}, st.chk, { background: set.done ? "#7FA36B" : "transparent", borderColor: set.done ? "#7FA36B" : "#3A4A3D" })} onClick={function() { handleSetDone(ex._idx, si); }}>{set.done ? "V" : ""}</button>
              ...
              {activeTimer && activeTimer.blockIndex === blk.blockIndex && <RestTimer key={activeTimer.blockIndex + "-" + Date.now()} seconds={activeTimer.seconds} onDone={function() { setActiveTimer(null); }} />}
            </div>); })}
        </div>
        <button style={st.addExBtn} onClick={addExercise}>+ ADD EXERCISE</button>
        <div style={st.actionRow}>
          <button style={Object.assign({}, st.completeBtn, { background: activeWorkout.completed ? "#7FA36B" : "#5A7A50" })} onClick={function() { updateWorkout(activeWorkout.date, activeWorkout.id, { completed: !activeWorkout.completed }); }}>{activeWorkout.completed ? "COMPLETED V" : "MARK COMPLETE"}</button>
  ...
  // MAIN VIEW
  // ============================================
  var dayName = DAYS[getDayIndex(selectedDate)]; var dayWorkouts = workouts[selectedDate] || []; var isToday = selectedDate === today;
  var md = getMonthDays(heatmapMonth.year, heatmapMonth.month); var startDay = md.startDay; var daysInMonth = md.daysInMonth;
  var monthLabel = new Date(heatmapMonth.year, heatmapMonth.month, 1).toLocaleString("en", { month: "long", year: "numeric" }).toUpperCase();

  return (
    <div style={st.shell}>
      <div style={st.header}>
        <div style={st.brandRow}><span style={st.brandIcon}>[O]</span><div><div style={st.brandTitle}>OPERATOR</div><div style={st.brandSub}>TRAINING LOG</div></div></div>
      ...
      {(tab === "day" || tab === "week") && (
        <div style={st.targetSection}><div style={st.targetGrid}>
          {WEEKLY_TARGETS.map(function(t) {
            var isMin = t.unit === "min"; var cur = isMin ? engineMin : (wgc[t.group] || 0); var tgt = t.target; var hit = cur === tgt; var over = cur > tgt;
            var pct = Math.min((cur / tgt) * 100, 100); var sc = over ? "#9FCB7B" : hit ? "#9FCB7B" : cur > 0 ? "#B98A52" : "#4A5A4D";
            return (<div key={t.group} style={st.targetCard}>
              <div style={st.targetHead}><span style={Object.assign({}, st.targetLabel, { color: t.color })}>{t.label}</span><span style={Object.assign({}, st.targetCount, { color: sc })}>{isMin ? cur + "/" + tgt + "m" : cur + "/" + tgt}</span></div>
      ...
      <div style={st.tabRow}>
        ...
        <button style={Object.assign({}, st.tabBtn, tab === "supps" ? st.tabActive : {})} onClick={function() { setTab("supps"); }}>SUPPS</button>
        <button style={Object.assign({}, st.tabBtn, tab === "insights" ? st.tabActive : {})} onClick={function() { setTab("insights"); }}>DATA</button>
      </div>
      ...
      {tab === "day" && (<>
        <div style={st.dayNav}>
          ...
          <button style={st.dayCenter} onClick={function() { if (!isToday) setSelectedDate(today); }}><div style={st.dayBig}>{dayName}</div><div style={st.dayDate}>{selectedDate.slice(5)}{isToday ? " -- TODAY" : ""}</div></button>
          <button style={st.navBtn} onClick={function() { shiftDay(1); }}>{">"}</button>
        </div>
        {dayWorkouts.length > 0 && (<div style={st.section}><div style={st.sectionTag}>LOGGED</div>
          {dayWorkouts.map(function(w) {
            ...
            return (<button key={w.id} style={Object.assign({}, st.loggedCard, { borderLeftColor: t ? t.color : "#2E3A31" })} onClick={function() { setActiveWorkout(w); }}>
              <div style={st.loggedTop}><span style={{ color: t ? t.color : "#999", fontSize: 12, fontWeight: 700 }}>{t ? t.icon : ""}</span><span style={st.loggedLabel}>{w.label}</span>{w.completed && <span style={st.loggedCheck}>V</span>}</div>
              <div style={st.loggedMeta}>{w.duration ? <span>{fmtDur(w.duration)}</span> : null}{tS > 0 && <span>{dS}/{tS} sets</span>}{w.exercises.length > 0 && <span>{w.exercises.length} ex</span>}</div>
              {w.notes && <div style={st.loggedNotes}>{w.notes}</div>}
            </button>);
          })}
        </div>)}
        {dayWorkouts.length === 0 && (<div style={st.emptyState}><div style={{ fontSize: 28, color: "#2E3A31" }}>-</div><div style={{ fontSize: 11, color: "#4A5A4D", letterSpacing: 3, marginTop: 4 }}>NO SESSIONS LOGGED</div></div>)}
      ...
      {/* WEEK */}
      {tab === "week" && (<>
        <div style={st.weekNav}><button style={st.navBtn} onClick={function() { shiftWeek(-1); }}>{"<"}</button><span style={st.weekRange}>{weekDates[0].slice(5)} -- {weekDates[6].slice(5)}</span><button style={st.navBtn} onClick={function() { shiftWeek(1); }}>{">"}</button></div>
        <div style={st.weekList}>
          {weekDates.map(function(dk, i) {
            var dw = workouts[dk] || []; var isTd = dk === today;
            return (<button key={dk} style={Object.assign({}, st.weekRow, { borderLeftColor: isTd ? "#7FA36B" : "transparent", background: isTd ? "#1A221C" : "transparent" })} onClick={function() { setSelectedDate(dk); setTab("day"); }}>
              <div style={st.weekDayCol}><span style={Object.assign({}, st.weekDayName, { color: isTd ? "#9FCB7B" : "#6A7A6D" })}>{DAYS[i]}</span><span style={Object.assign({}, st.weekDayNum, { color: isTd ? "#D7E0D2" : "#4A5A4D" })}>{dk.split("-")[2]}</span></div>
              <div style={st.weekContent}>
                {dw.length > 0 ? (<div style={st.weekChips}>{dw.map(function(w) { var t = WORKOUT_TYPES.find(function(x) { return x.id === w.type; }); return (
                  <span key={w.id} style={Object.assign({}, st.weekChip, { borderColor: (t ? t.color : "#999") + "55", background: (t ? t.color : "#999") + "10" })}><span style={{ color: t ? t.color : "#999", fontSize: 10, fontWeight: 700 }}>{t ? t.short : ""}</span>{w.completed && <span style={{ color: "#9FCB7B", fontSize: 10, marginLeft: 3 }}>V</span>}</span>); })}</div>
      ...
      {tab === "supps" && (
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <div style={st.insightTitle}>DAILY SUPPLEMENTS</div>
            <span style={{ fontSize: 12, fontWeight: 700, color: suppDone === suppTotal ? "#9FCB7B" : "#6A7A6D" }}>{suppDone}/{suppTotal}</span>
          </div>
          {SUPP_SCHEDULE.map(function(group) {
            var groupDone = group.items.filter(function(it) { return suppChecks[it.id]; }).length;
            var groupAll = group.items.length;
            return (
              <div key={group.time} style={st.suppGroup}>
                <div style={st.suppGroupHeader}>
                  <span style={st.suppGroupTitle}>{group.time}</span>
                  <span style={{ fontSize: 9, color: groupDone === groupAll ? "#9FCB7B" : "#4A5A4D" }}>{groupDone}/{groupAll}</span>
                </div>
                {group.items.map(function(item) {
                  var checked = suppChecks[item.id] || false;
                  return (
                    <button key={item.id} style={Object.assign({}, st.suppItem, checked ? st.suppItemDone : {})} onClick={function() { toggleSupp(item.id); }}>
                      <div style={Object.assign({}, st.suppChk, { background: checked ? "#7FA36B" : "transparent", borderColor: checked ? "#7FA36B" : "#3A4A3D" })}>{checked ? "V" : ""}</div>
                      <div style={{ flex: 1 }}>
                        <div style={Object.assign({}, st.suppName, checked ? { textDecoration: "line-through", color: "#4A5A4D" } : {})}>{item.name}</div>
                        <div style={st.suppDose}>{item.dose}</div>
                        {item.note && <div style={st.suppNote}>{item.note}</div>}
                      </div>
                    </button>
                  );
                })}
          ...
          <div style={{ textAlign: "center", padding: "16px 0", fontSize: 9, color: "#3A4A3D", letterSpacing: 1 }}>Resets daily at midnight</div>
        </div>
      )}
      ...
      {tab === "insights" && (
        <div>
          ...
          {insightSection === "weight" && (<div>
            <div style={st.insightTitle}>BODYWEIGHT TRACKER</div>
            <div style={st.bwForm}><input type="date" style={st.bwDateIn} value={bwDateInput} onChange={function(e) { setBwDateInput(e.target.value); }} /><input type="number" step="0.1" style={st.bwWeightIn} placeholder="kg" value={bwInput} onChange={function(e) { setBwInput(e.target.value); }} onKeyDown={function(e) { if (e.key === "Enter") addBodyweight(); }} /><button style={st.bwAddBtn} onClick={addBodyweight}>LOG</button></div>
            {bodyweightData.length >= 2 && (<div style={st.chartWrap}><ResponsiveContainer width="100%" height={180}><LineChart data={bodyweightData.map(function(e) { return Object.assign({}, e, { d: e.date.slice(5) }); })}><XAxis dataKey="d" tick={{ fontSize: 9, fill: "#6A7A6D" }} axisLine={{ stroke: "#2E3A31" }} tickLine={false} /><YAxis domain={["dataMin - 1", "dataMax + 1"]} tick={{ fontSize: 9, fill: "#6A7A6D" }} axisLine={{ stroke: "#2E3A31" }} tickLine={false} width={35} /><Tooltip contentStyle={{ background: "#1A221C", border: "1px solid #2E3A31", fontSize: 11, color: "#D7E0D2" }} formatter={function(v) { return [v + " kg", "Weight"]; }} /><Line type="monotone" dataKey="weight" stroke="#9FCB7B" strokeWidth={2} dot={{ r: 3, fill: "#9FCB7B" }} /></LineChart></ResponsiveContainer></div>)}
            ...
            {bodyweightData.slice().reverse().slice(0, 15).map(function(e) { return <div key={e.date} style={st.bwRow}><span style={st.bwRowDate}>{e.date}</span><span style={st.bwRowVal}>{e.weight} kg</span><button style={st.bwRowDel} onClick={function() { deleteBodyweight(e.date); }}>x</button></div>; })}
          </div>)}
          {insightSection === "streaks" && (<div>
            ...
            <div style={st.sectionTag}>PERFECT WEEKS BY TYPE</div><div style={st.emptyHint}>Week counts as perfect if target is met or exceeded</div>
            {WEEKLY_TARGETS.map(function(t) { var d = perfectWeeks[t.group] || { perfect: 0, total: 0 }; var pct = d.total > 0 ? Math.round((d.perfect / d.total) * 100) : 0; return (<div key={t.group} style={st.perfectRow}><div style={st.perfectHead}><span style={Object.assign({}, st.perfectLabel, { color: t.color })}>{t.label}</span><span style={st.perfectCount}>{d.perfect} / {d.total}</span></div><div style={st.targetBarBg}><div style={Object.assign({}, st.targetBarFill, { width: pct + "%", background: t.color })} /></div><div style={st.perfectPct}>{pct}% HIT RATE</div></div>); })}
          </div>)}
          {insightSection === "totals" && (<div>
            <div style={st.insightTitle}>ALL-TIME TOTALS</div>
            {[{ label: "GYM", ids: ["upper-press","lower","upper-pump","total"], color: "#9FCB7B" },{ label: "BJJ", ids: ["bjj-gi","bjj-nogi"], color: "#6BA8A0" },{ label: "BOXING", ids: ["boxing"], color: "#C4885A" },{ label: "ENGINE", ids: ["engine"], color: "#6B9A8A" },{ label: "MOBILITY", ids: ["mobility"], color: "#8A8E78" }].map(function(g) { var ses = g.ids.reduce(function(s, id) { return s + (historicalTotals[id] ? historicalTotals[id].sessions : 0); }, 0); var min = g.ids.reduce(function(s, id) { return s + (historicalTotals[id] ? historicalTotals[id].minutes : 0); }, 0); return (<div key={g.label} style={st.totalRow}><span style={Object.assign({}, st.totalLabel, { color: g.color })}>{g.label}</span><div style={st.totalNums}><span style={st.totalBig}>{ses}</span><span style={st.totalUnit}>sessions</span>{min > 0 && <><span style={Object.assign({}, st.totalBig, { marginLeft: 12 })}>{fmtDur(min)}</span><span style={st.totalUnit}>total</span></>}</div></div>); })}
            <div style={Object.assign({}, st.sectionTag, { marginTop: 20 })}>BREAKDOWN BY TYPE</div>
            {WORKOUT_TYPES.filter(function(t) { return historicalTotals[t.id] && historicalTotals[t.id].sessions > 0; }).map(function(t) { var d = historicalTotals[t.id]; return (<div key={t.id} style={st.breakdownRow}><span style={{ color: t.color, fontSize: 12, fontWeight: 700 }}>{t.icon}</span><span style={st.breakdownLabel}>{t.label}</span><span style={st.breakdownVal}>{d.sessions}x</span>{d.minutes > 0 && <span style={st.breakdownMin}>{fmtDur(d.minutes)}</span>}</div>); })}
            {Object.values(historicalTotals).every(function(d) { return d.sessions === 0; }) && <div style={st.emptyHint}>No workout data yet</div>}
          </div>)}
          {insightSection === "heatmap" && (<div>
            <div style={st.insightTitle}>TRAINING CALENDAR</div>
            <div style={st.heatNav}><button style={st.navBtn} onClick={function() { var m = heatmapMonth.month === 0 ? 11 : heatmapMonth.month - 1; setHeatmapMonth({ year: heatmapMonth.month === 0 ? heatmapMonth.year - 1 : heatmapMonth.year, month: m }); }}>{"<"}</button><span style={st.heatLabel}>{monthLabel}</span><button style={st.navBtn} onClick={function() { var m = heatmapMonth.month === 11 ? 0 : heatmapMonth.month + 1; setHeatmapMonth({ year: heatmapMonth.month === 11 ? heatmapMonth.year + 1 : heatmapMonth.year, month: m }); }}>{">"}</button></div>
            <div style={st.heatGrid}>
              ...
              {Array.from({ length: daysInMonth }, function(_, i) { var day = i + 1; var ds = heatmapMonth.year + "-" + String(heatmapMonth.month + 1).padStart(2, "0") + "-" + String(day).padStart(2, "0"); var cnt = (workouts[ds] || []).length; var hw = cnt > 0; var isTd = ds === today; return <div key={day} style={Object.assign({}, st.heatCell, { background: hw ? cnt >= 3 ? "#7FA36B" : cnt >= 2 ? "rgba(127,163,107,0.45)" : "rgba(127,163,107,0.2)" : "#1A221C", border: isTd ? "1px solid #9FCB7B" : "1px solid #2E3A31" })}><span style={{ fontSize: 10, color: hw ? "#D7E0D2" : "#3A4A3D" }}>{day}</span></div>; })}
            </div>
            <div style={st.heatLegend}><span style={st.heatLegItem}><span style={Object.assign({}, st.heatLegDot, { background: "#1A221C" })} />Rest</span><span style={st.heatLegItem}><span style={Object.assign({}, st.heatLegDot, { background: "rgba(127,163,107,0.2)" })} />1</span><span style={st.heatLegItem}><span style={Object.assign({}, st.heatLegDot, { background: "rgba(127,163,107,0.45)" })} />2</span><span style={st.heatLegItem}><span style={Object.assign({}, st.heatLegDot, { background: "#7FA36B" })} />3+</span></div>
          </div>)}
          {insightSection === "volume" && (<div>
            <div style={st.insightTitle}>VOLUME LIFTED</div>
            <div style={st.streakCard}><div style={st.streakBig}>{totalVolume > 1000 ? (totalVolume / 1000).toFixed(1) + "t" : totalVolume + "kg"}</div><div style={st.streakLabel}>ALL-TIME TOTAL</div>{totalVolume > 0 && <div style={{ marginTop: 10, fontSize: 10, color: "#6A7A6D", letterSpacing: 1 }}>{getWeightComparison(totalVolume)}</div>}</div>
            {weeklyVolume.length > 0 ? (<><div style={st.sectionTag}>WEEKLY VOLUME (KG)</div><div style={st.chartWrap}><ResponsiveContainer width="100%" height={180}><BarChart data={weeklyVolume.slice(-12)}><XAxis dataKey="week" tick={{ fontSize: 9, fill: "#6A7A6D" }} axisLine={{ stroke: "#2E3A31" }} tickLine={false} /><YAxis tick={{ fontSize: 9, fill: "#6A7A6D" }} axisLine={{ stroke: "#2E3A31" }} tickLine={false} width={40} tickFormatter={function(v) { return v > 1000 ? (v / 1000).toFixed(0) + "k" : v; }} /><Tooltip contentStyle={{ background: "#1A221C", border: "1px solid #2E3A31", fontSize: 11, color: "#D7E0D2" }} formatter={function(v) { return [v.toLocaleString() + " kg", "Volume"]; }} /><Bar dataKey="volume" radius={[2, 2, 0, 0]}>{weeklyVolume.slice(-12).map(function(_, i) { return <Cell key={i} fill="#7FA36B" />; })}</Bar></BarChart></ResponsiveContainer></div><div style={st.sectionTag}>RECENT WEEKS</div>{weeklyVolume.slice().reverse().slice(0, 8).map(function(w) { return <div key={w.week} style={st.volRow}><span style={st.volWeek}>w/{w.week}</span><span style={st.volVal}>{w.volume.toLocaleString()} kg</span></div>; })}</>
      ...
      {logModal && (
        <div style={st.overlay} onClick={function() { setLogModal(false); }}>
          <div style={st.modal} onClick={function(e) { e.stopPropagation(); }}>
            ...
            <div style={st.modalGroup}><div style={st.modalGroupLabel}>GYM -- HC 1.2 PHASE 1</div>
              {WORKOUT_TYPES.filter(function(t) { return t.group === "gym"; }).map(function(type) { var tmpl = BLOCK_TEMPLATES[type.id]; return (<button key={type.id} style={Object.assign({}, st.modalItem, { borderLeftColor: type.color })} onClick={function() { addWorkout(selectedDate, type.id); }}><span style={{ color: type.color, fontSize: 14, fontWeight: 700 }}>{type.icon}</span><div><div style={st.modalItemLabel}>{type.label}</div>{tmpl && <div style={{ fontSize: 9, color: "#5A6A5D", marginTop: 1 }}>{tmpl.length} blocks</div>}</div></button>); })}
            </div>
            <div style={st.modalGroup}><div style={st.modalGroupLabel}>COMBAT / CONDITIONING</div>
              {WORKOUT_TYPES.filter(function(t) { return ["bjj", "boxing", "engine"].indexOf(t.group) >= 0; }).map(function(type) { return (<button key={type.id} style={Object.assign({}, st.modalItem, { borderLeftColor: type.color })} onClick={function() { addWorkout(selectedDate, type.id); }}><span style={{ color: type.color, fontSize: 14, fontWeight: 700 }}>{type.icon}</span><div><div style={st.modalItemLabel}>{type.label}</div>{type.group === "engine" && <div style={{ fontSize: 9, color: "#6B9A8A", marginTop: 1 }}>Log duration for 90min target</div>}</div></button>); })}
            </div>
            <div style={st.modalGroup}><div style={st.modalGroupLabel}>OTHER</div>
              {WORKOUT_TYPES.filter(function(t) { return t.group === "other"; }).map(function(type) { return (<button key={type.id} style={Object.assign({}, st.modalItem, { borderLeftColor: type.color })} onClick={function() { addWorkout(selectedDate, type.id); }}><span style={{ color: type.color, fontSize: 14, fontWeight: 700 }}>{type.icon}</span><div><div style={st.modalItemLabel}>{type.label}</div></div></button>); })}
            </div>
...
var st = {
  ...
  tabBtn: { flex: 1, background: "none", border: "none", borderBottom: "2px solid #252F28", color: "#4A5A4D", fontFamily: "inherit", fontSize: 9, letterSpacing: 1, padding: "10px 0", cursor: "pointer", fontWeight: 700 },
  ...
  // Supplement styles
  suppGroup: { marginBottom: 16 },
  suppGroupHeader: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", background: "#202A22", border: "1px solid #2E3A31", borderBottom: "none", marginBottom: 0 },
  suppGroupTitle: { fontSize: 9, fontWeight: 700, letterSpacing: 2, color: "#D7E0D2" },
  suppItem: { display: "flex", alignItems: "flex-start", gap: 10, padding: "12px", border: "1px solid #2E3A31", borderTop: "none", background: "#1A221C", cursor: "pointer", fontFamily: "inherit", width: "100%", boxSizing: "border-box", textAlign: "left" },
  suppItemDone: { background: "#161E18" },
  suppChk: { width: 24, height: 24, border: "1px solid #3A4A3D", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "#D7E0D2", flexShrink: 0, marginTop: 1 },
  suppName: { fontSize: 12, fontWeight: 700, color: "#D7E0D2", letterSpacing: 1 },
  suppDose: { fontSize: 10, color: "#6A7A6D", marginTop: 2, letterSpacing: 1 },
  suppNote: { fontSize: 9, color: "#4A5A4D", marginTop: 2, fontStyle: "italic" },
};
